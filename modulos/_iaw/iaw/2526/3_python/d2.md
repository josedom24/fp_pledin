...
* Mostrar una página web dínamica a partir de una plantilla a la que le manda información, usando la instrucción `render`. En nuestro caso, la función `inicio` muestra una página web usando el template `inicio.html` al que le mandadmos la lista de mensajes que tenemos guardado en la base de datos:
...
3. Da de alta una encuesta en la zona de administración y comprueba la funcionalidad de la aplicación.
