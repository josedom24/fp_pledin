...
4. El inventario es el fichero donde definimos los equipos que vamos a configurar. En nuestro caso se llama `hosts`. En el inventario se clasifican los equipos por grupos:
	...
	* A la máquina la hemos llamado `nodo1` (**cambia el nombre y pon el de tu máquina**), además **debes rellenar la siguiente información del nodo**:
		...
		* `ansible_ssh_user`: Usuario sin privilegios que hemos creado en ele equipo en el punto 2.
...
8. Veamos el playbook que se encuentra en el fichero `site.yml`:
	...
	* Primera línea: `hosts: all`. _Significa que las tareas se van a ejecutar en todos los nodos definidos en el inventario. podríamos haber puesto `hosts: servidores` o `hosts: nodo1`.
	* `become: true`: En las tareas se que necesite ejecutarlas como administrador se utilizará `sudo`. 
...
9. Ejecuta el playbook:
	...
	* **Comprobación del funcionamiento: Comprueba que se ha copiado un fichero `foo.txt` en el servidor, accede desde un navegador al serviidor y comprueba que aprece el fichero `index.html` que hemos creado.**
