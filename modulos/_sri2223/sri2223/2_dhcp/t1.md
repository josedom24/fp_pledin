...
4. Vamos a estudiar el fichero de configuración del servicio `/etc/dhcp/dhcpd.conf`.
	...
	* `default-lease-time`: Es el tiempo por defecto en segundos de concesión que se le asignará a un cliente en caso de que éste no haya solicitado ningún periodo en concreto. Estos tres primeros parámetro determinan el **tiempo TE o tiempo de concesión**.
